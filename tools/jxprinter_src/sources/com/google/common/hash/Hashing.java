...
public final class Hashing {
    ...
    public static final class LinearCongruentialGenerator {
        ...
        public LinearCongruentialGenerator(long j6) {
            this.state = j6;
        }

        public double nextDouble() {
            long j6 = (this.state * MurmurHash3.NULL_HASHCODE) + 1;
            this.state = j6;
            return ((double) (((int) (j6 >>> 33)) + 1)) / 2.147483648E9d;
        }
    }
    ...
    public static int checkPositiveAndMakeMultipleOf32(int i5) {
    ...
    public static HashCode combineOrdered(Iterable<HashCode> iterable) {
        ...
        while (it2.hasNext()) {
            ...
            for (int i5 = 0; i5 < bArrAsBytes.length; i5++) {
                bArr[i5] = (byte) ((bArr[i5] * 37) ^ bArrAsBytes[i5]);
            }
        }
        return HashCode.fromBytesNoCopy(bArr);
    }
    ...
    public static HashCode combineUnordered(Iterable<HashCode> iterable) {
        ...
        while (it2.hasNext()) {
            ...
            Preconditions.checkArgument(bArrAsBytes.length == iBits, "All hashcodes must have the same bit length.");
            for (int i5 = 0; i5 < bArrAsBytes.length; i5++) {
                bArr[i5] = (byte) (bArr[i5] + bArrAsBytes[i5]);
            }
        }
        return HashCode.fromBytesNoCopy(bArr);
    }
    ...
    public static int consistentHash(HashCode hashCode, int i5) {
        return consistentHash(hashCode.padToLong(), i5);
    }
    ...
    public static HashFunction goodFastHash(int i5) {
        int iCheckPositiveAndMakeMultipleOf32 = checkPositiveAndMakeMultipleOf32(i5);
    ...
    private static String hmacToString(String str, Key key) {
        ...
        return AbstractC0157z.s(a.u("Hashing.", str, "(Key[algorithm=", algorithm, ", format="), key.getFormat(), "])");
    }
    ...
    public static HashFunction murmur3_128(int i5) {
        return new Murmur3_128HashFunction(i5);
    }

    @Deprecated
    public static HashFunction murmur3_32(int i5) {
        return new Murmur3_32HashFunction(i5, false);
    }

    public static HashFunction murmur3_32_fixed(int i5) {
        return new Murmur3_32HashFunction(i5, true);
    }
    ...
    public static int consistentHash(long j6, int i5) {
        int i6 = 0;
        Preconditions.checkArgument(i5 > 0, "buckets must be positive: %s", i5);
        LinearCongruentialGenerator linearCongruentialGenerator = new LinearCongruentialGenerator(j6);
        while (true) {
            int iNextDouble = (int) (((double) (i6 + 1)) / linearCongruentialGenerator.nextDouble());
            if (iNextDouble < 0 || iNextDouble >= i5) {
                break;
            }
            i6 = iNextDouble;
        }
    ...
    public static HashFunction sipHash24(long j6, long j7) {
