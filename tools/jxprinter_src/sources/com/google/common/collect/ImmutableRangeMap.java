...
public class ImmutableRangeMap<K extends Comparable<?>, V> implements RangeMap<K, V>, Serializable {
    ...
    public static final class Builder<K extends Comparable<?>, V> {
        ...
        public ImmutableRangeMap<K, V> build() {
            ...
            for (int i5 = 0; i5 < this.entries.size(); i5++) {
                Range<K> key = this.entries.get(i5).getKey();
                if (i5 > 0) {
                    Range<K> key2 = this.entries.get(i5 - 1).getKey();
                    if (key.isConnected(key2) && !key.intersection(key2).isEmpty()) {
                        String strValueOf = String.valueOf(key2);
                        String strValueOf2 = String.valueOf(key);
                        throw new IllegalArgumentException(com.google.android.gms.auth.api.accounttransfer.a.j(strValueOf2.length() + strValueOf.length() + 47, "Overlapping ranges: range ", strValueOf, " overlaps with entry ", strValueOf2));
                    }
                }
                builder.add(key);
                builder2.add(this.entries.get(i5).getValue());
            }
        ...
        @CanIgnoreReturnValue
        public Builder<K, V> put(Range<K> range, V v6) {
            Preconditions.checkNotNull(range);
            Preconditions.checkNotNull(v6);
            Preconditions.checkArgument(!range.isEmpty(), "Range must not be empty, but was %s", range);
            this.entries.add(Maps.immutableEntry(range, v6));
            return this;
        }
    ...
    @Override // com.google.common.collect.RangeMap
    public V get(K k6) {
        int iBinarySearch = SortedLists.binarySearch(this.ranges, (Function<? super E, Cut>) Range.lowerBoundFn(), Cut.belowValue(k6), SortedLists.KeyPresentBehavior.ANY_PRESENT, SortedLists.KeyAbsentBehavior.NEXT_LOWER);
        if (iBinarySearch != -1 && this.ranges.get(iBinarySearch).contains(k6)) {
    ...
    public ImmutableRangeMap<K, V> subRangeMap(final Range<K> range) {
        ...
        final int i5 = iBinarySearch2 - iBinarySearch;
        return (ImmutableRangeMap<K, V>) new ImmutableRangeMap<K, V>(this, new ImmutableList<Range<K>>() { // from class: com.google.common.collect.ImmutableRangeMap.1
            ...
            public Range<K> get(int i6) {
                ...
                return (i6 == 0 || i6 == i5 + (-1)) ? ((Range) ImmutableRangeMap.this.ranges.get(i6 + iBinarySearch)).intersection(range) : (Range) ImmutableRangeMap.this.ranges.get(i6 + iBinarySearch);
            }
