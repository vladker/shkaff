...
class FilteredEntryMultimap<K, V> extends AbstractMultimap<K, V> implements FilteredMultimap<K, V> {
    ...
    public class Keys extends Multimaps.Keys<K, V> {
        ...
        public int remove(Object obj, int i5) {
            ...
            Iterator<V> it = collection.iterator();
            while (it.hasNext()) {
                if (FilteredEntryMultimap.this.satisfies(obj, it.next()) && (i6 = i6 + 1) <= i5) {
                    it.remove();
                }
            }
            return i6;
    ...
    public final class ValuePredicate implements Predicate<V> {
        ...
        public ValuePredicate(K k6) {
            this.key = k6;
        }

        @Override // com.google.common.base.Predicate
        public boolean apply(@ParametricNullness V v6) {
            return FilteredEntryMultimap.this.satisfies(this.key, v6);
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public boolean satisfies(@ParametricNullness K k6, @ParametricNullness V v6) {
        return this.predicate.apply(Maps.immutableEntry(k6, v6));
    }
    ...
    public Collection<V> get(@ParametricNullness K k6) {
        return filterCollection(this.unfiltered.get(k6), new ValuePredicate(k6));
    }
