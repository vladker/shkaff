...
public abstract class ForwardingExecutorService extends ForwardingObject implements ExecutorService, AutoCloseable {
    ...
    public boolean awaitTermination(long j6, TimeUnit timeUnit) {
        return delegate().awaitTermination(j6, timeUnit);
    }
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> collection, long j6, TimeUnit timeUnit) {
        return delegate().invokeAll(collection, j6, timeUnit);
    }

    @Override // java.util.concurrent.ExecutorService
    public <T> T invokeAny(Collection<? extends Callable<T>> collection, long j6, TimeUnit timeUnit) {
        return (T) delegate().invokeAny(collection, j6, timeUnit);
    }
    ...
    @Override // java.util.concurrent.ExecutorService
    public <T> Future<T> submit(Runnable runnable, @ParametricNullness T t6) {
        return delegate().submit(runnable, t6);
    }
}
