...
public class SessionReportingCoordinator {
    ...
    private ApplicationExitInfo findRelevantApplicationExitInfo(String str, List<ApplicationExitInfo> list) {
        ...
        while (it.hasNext()) {
            ApplicationExitInfo applicationExitInfoE = p.e(it.next());
            if (applicationExitInfoE.getTimestamp() < startTimestampMillis) {
                return null;
            }
            if (applicationExitInfoE.getReason() == 6) {
                return applicationExitInfoE;
            }
        }
        return null;
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void lambda$persistEvent$0(CrashlyticsReport.Session.Event event, EventMetadata eventMetadata, boolean z6) {
        Logger.getLogger().d("disk worker: log non-fatal event to persistence");
        this.reportPersistence.persistEvent(event, eventMetadata.getSessionId(), z6);
    }
    ...
    private void persistEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull String str, @NonNull EventMetadata eventMetadata, boolean z6) {
        boolean zEquals = str.equals("crash");
        CrashlyticsReport.Session.Event eventAddMetaDataToEvent = addMetaDataToEvent(this.dataCapture.captureEventData(th, thread, str, eventMetadata.getTimestamp(), 4, 8, z6), eventMetadata.getAdditionalCustomKeys());
        if (z6) {
            this.reportPersistence.persistEvent(eventAddMetaDataToEvent, eventMetadata.getSessionId(), zEquals);
        } else {
            this.crashlyticsWorkers.diskWrite.submit(new com.appdev.standard.page.printerlabel.util.c(1, this, eventAddMetaDataToEvent, eventMetadata, zEquals));
        }
    }
    ...
    public void finalizeSessions(long j6, @Nullable String str) {
        this.reportPersistence.finalizeReports(str, j6);
    }
    ...
    public void onBeginSession(@NonNull String str, long j6) {
        this.reportPersistence.persistReport(this.dataCapture.captureReportData(str, j6));
    }
    ...
    public void persistFatalEvent(@NonNull Throwable th, @NonNull Thread thread, @NonNull String str, long j6) {
        ...
        persistEvent(th, thread, "crash", new EventMetadata(str, j6), true);
    }
    ...
    public Task<Void> sendReports(@NonNull Executor executor, @Nullable String str) {
        ...
        for (CrashlyticsReportWithSessionId crashlyticsReportWithSessionId : listLoadFinalizedReports) {
            if (str == null || str.equals(crashlyticsReportWithSessionId.getSessionId())) {
                arrayList.add(this.reportsSender.enqueueReport(ensureHasFid(crashlyticsReportWithSessionId), str != null).continueWith(executor, new Y2.a(this, 14)));
            }
        }
