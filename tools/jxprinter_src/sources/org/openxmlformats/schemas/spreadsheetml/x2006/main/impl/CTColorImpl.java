...
public class CTColorImpl extends XmlComplexContentImpl implements CTColor {
    ...
    @Override // org.openxmlformats.schemas.spreadsheetml.x2006.main.CTColor
    public void setIndexed(long j6) {
        synchronized (monitor()) {
            try {
                check_orphaned();
                TypeStore typeStore = get_store();
    ...
    public STUnsignedIntHex xgetRgb() {
        ...
        synchronized (monitor()) {
            check_orphaned();
            sTUnsignedIntHex = (STUnsignedIntHex) get_store().find_attribute_user(PROPERTY_QNAME[2]);
        }
    ...
    public XmlDouble xgetTint() {
        ...
        synchronized (monitor()) {
            try {
                ...
                TypeStore typeStore = get_store();
                QName[] qNameArr = PROPERTY_QNAME;
                xmlDouble = (XmlDouble) typeStore.find_attribute_user(qNameArr[4]);
                if (xmlDouble == null) {
                    xmlDouble = (XmlDouble) get_default_attribute_value(qNameArr[4]);
                }
            } catch (Throwable th) {
                throw th;
            }
    ...
    public void xsetRgb(STUnsignedIntHex sTUnsignedIntHex) {
        synchronized (monitor()) {
            try {
                ...
                TypeStore typeStore = get_store();
                QName[] qNameArr = PROPERTY_QNAME;
                STUnsignedIntHex sTUnsignedIntHex2 = (STUnsignedIntHex) typeStore.find_attribute_user(qNameArr[2]);
                if (sTUnsignedIntHex2 == null) {
                    sTUnsignedIntHex2 = (STUnsignedIntHex) get_store().add_attribute_user(qNameArr[2]);
                }
