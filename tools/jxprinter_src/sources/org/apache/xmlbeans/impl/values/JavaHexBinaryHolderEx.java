...
public abstract class JavaHexBinaryHolderEx extends JavaHexBinaryHolder {
    ...
    public JavaHexBinaryHolderEx(SchemaType schemaType, boolean z6) {
    ...
    public static void validateValue(byte[] bArr, SchemaType schemaType, ValidationContext validationContext) {
        ...
        if (facet != null && (iIntValue3 = ((XmlObjectBase) facet).getBigIntegerValue().intValue()) != bArr.length) {
            validationContext.invalid(XmlErrorCodes.DATATYPE_LENGTH_VALID$BINARY, new Object[]{XmlErrorCodes.HEXBINARY, Integer.valueOf(bArr.length), Integer.valueOf(iIntValue3), QNameHelper.readable(schemaType)});
        }
        ...
        if (facet2 != null && (iIntValue2 = ((XmlObjectBase) facet2).getBigIntegerValue().intValue()) > bArr.length) {
            validationContext.invalid(XmlErrorCodes.DATATYPE_MIN_LENGTH_VALID$BINARY, new Object[]{XmlErrorCodes.HEXBINARY, Integer.valueOf(bArr.length), Integer.valueOf(iIntValue2), QNameHelper.readable(schemaType)});
        }
        ...
        if (facet3 != null && (iIntValue = ((XmlObjectBase) facet3).getBigIntegerValue().intValue()) < bArr.length) {
            validationContext.invalid(XmlErrorCodes.DATATYPE_MAX_LENGTH_VALID$BINARY, new Object[]{XmlErrorCodes.HEXBINARY, Integer.valueOf(bArr.length), Integer.valueOf(iIntValue), QNameHelper.readable(schemaType)});
        }
    ...
    public void set_text(String str) {
        byte[] bArrValidateLexical = _validateOnSet() ? JavaHexBinaryHolder.validateLexical(str, schemaType(), XmlObjectBase._voorVc) : JavaHexBinaryHolder.lex(str, XmlObjectBase._voorVc);
        if (_validateOnSet() && bArrValidateLexical != null) {
        ...
        this._value = bArrValidateLexical;
    }

    @Override // org.apache.xmlbeans.impl.values.XmlObjectBase
    public void validate_simpleval(String str, ValidationContext validationContext) {
        JavaHexBinaryHolder.validateLexical(str, schemaType(), validationContext);
