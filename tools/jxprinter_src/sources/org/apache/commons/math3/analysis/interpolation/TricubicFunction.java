...
class TricubicFunction implements TrivariateFunction {
    ...
    private static final short f6744N = 4;

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    private final double[][][] f6745a = (double[][][]) Array.newInstance((Class<?>) Double.TYPE, 4, 4, 4);

    public TricubicFunction(double[] dArr) {
        for (int i5 = 0; i5 < 4; i5++) {
            for (int i6 = 0; i6 < 4; i6++) {
                for (int i7 = 0; i7 < 4; i7++) {
                    this.f6745a[i5][i6][i7] = dArr[(((i7 * 4) + i6) * 4) + i5];
                }
            }
        }
    }
    ...
    public double value(double d, double d6, double d7) {
        ...
        for (int i5 = 0; i5 < 4; i5++) {
            for (int i6 = 0; i6 < 4; i6++) {
                for (int i7 = 0; i7 < 4; i7++) {
                    d8 += this.f6745a[i5][i6][i7] * dArr[i5] * dArr2[i6] * dArr3[i7];
                }
            }
        }
        return d8;
