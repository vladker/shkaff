...
public class LocalVariableTableAttribute extends BCIRenumberedAttribute {
    ...
    public LocalVariableTableAttribute(int i5, int[] iArr, int[] iArr2, CPUTF8[] cputf8Arr, CPUTF8[] cputf8Arr2, int[] iArr3) {
        super(attributeName);
        this.local_variable_table_length = i5;
        this.start_pcs = iArr;
        this.lengths = iArr2;
    ...
    public ClassFileEntry[] getNestedClassFileEntries() {
        ...
        for (int i5 = 0; i5 < this.local_variable_table_length; i5++) {
            arrayList.add(this.names[i5]);
            arrayList.add(this.descriptors[i5]);
        }
    ...
    public void resolve(ClassConstantPool classConstantPool) {
        ...
        int i5 = this.local_variable_table_length;
        this.name_indexes = new int[i5];
        this.descriptor_indexes = new int[i5];
        for (int i6 = 0; i6 < this.local_variable_table_length; i6++) {
            this.names[i6].resolve(classConstantPool);
            this.descriptors[i6].resolve(classConstantPool);
            this.name_indexes[i6] = classConstantPool.indexOf(this.names[i6]);
            this.descriptor_indexes[i6] = classConstantPool.indexOf(this.descriptors[i6]);
        }
    }
    ...
    public String toString() {
        return AbstractC0157z.l(" variables", this.local_variable_table_length, new StringBuilder("LocalVariableTable: "));
    }
    ...
    public void writeBody(DataOutputStream dataOutputStream) throws IOException {
        ...
        for (int i5 = 0; i5 < this.local_variable_table_length; i5++) {
            ...
            dataOutputStream.writeShort(this.descriptor_indexes[i5]);
